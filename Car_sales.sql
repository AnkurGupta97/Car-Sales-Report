...
select sum(price)as a from car_sales where EXTRACT(YEAR FROM Date)='2020';



SELECT DATE_FORMAT(Date, '%d/%m/%Y') FROM car_sales;
select * from car_sales where year(Date) = 2020;
